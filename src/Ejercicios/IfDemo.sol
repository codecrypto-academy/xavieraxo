...
contract IfDemo {
    function numeroGanador(int n) external pure returns (string memory) {
        return n == 7 ? "Has ganado" : "Has perdido";
    }
    function valorAbsoluto(int n) external pure returns (int) { return n < 0 ? -n : n; }
    function esParDeTres(uint n) external pure returns (bool) { return n % 2 == 0 && n >= 100 && n <= 999; }
